...
class DownloadTaskImpl implements DownloadTask, OnHttpDownloadListener, OnFileSaveListener, OnRangeChangeListener {
    ...
    private boolean checkTaskCanExecute() {
        ...
        if (mTaskParamInfo == null) {
            ...
            failReason = new FileDownloadStatusFailReason(url, "init param is null pointer !",
                    FileDownloadStatusFailReason.TYPE_NULL_POINTER);
        }
        if (failReason == null && !UrlUtil.isUrl(url)) {
            // error, url illegal
            failReason = new FileDownloadStatusFailReason(url, "url illegal !", FileDownloadStatusFailReason
                    .TYPE_URL_ILLEGAL);
        }
        if (failReason == null && !FileUtil.isFilePath(mTaskParamInfo.getFilePath())) {
            // error, saveDir illegal
            failReason = new FileDownloadStatusFailReason(url, "saveDir illegal !", FileDownloadStatusFailReason
        ...
        if (failReason == null && (!FileUtil.canWrite(mTaskParamInfo.getTempFilePath()) || !FileUtil.canWrite
            ...
            failReason = new FileDownloadStatusFailReason(url, "savePath can not write !",
                    FileDownloadStatusFailReason.TYPE_STORAGE_SPACE_CAN_NOT_WRITE);
        }
        ...
        if (failReason == null) {
            try {
                ...
                if (!FileUtil.isFilePath(checkPath)) {
                    failReason = new FileDownloadStatusFailReason(url, "file save path illegal !",
                            FileDownloadStatusFailReason.TYPE_FILE_SAVE_PATH_ILLEGAL);
                } else {
                    ...
                    if (freeSize == -1 || needDownloadSize > freeSize) {
                        ...
                        failReason = new FileDownloadStatusFailReason(url, "storage space is full or" +
                                " storage can not " + "write !", FileDownloadStatusFailReason
                                .TYPE_STORAGE_SPACE_IS_FULL);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
                // cast Exception to FileDownloadStatusFailReason
                failReason = new FileDownloadStatusFailReason(url, e);
            }
        }
        ...
        try {
            ...
            // ------------start checking conditions------------
            {
                ...
                if (!UrlUtil.isUrl(url)) {
                    // error url illegal
                    FileDownloadStatusFailReason failReason = new FileDownloadStatusFailReason(url, "url illegal "
            ...
            mFinishState = new FinishState(status, new FileDownloadStatusFailReason(url, e));
        } finally {
            ...
            // ------------start checking mFinishState------------
            {
                DownloadFileInfo downloadFileInfo = getDownloadFile();
                if (downloadFileInfo == null) {
                    FileDownloadStatusFailReason failReason = new FileDownloadStatusFailReason(url, "the " +
                            "DownloadFile " +
                            "is" + " null, may be not deleted ?", FileDownloadStatusFailReason.TYPE_NULL_POINTER);
                    mFinishState = new FinishState(Status.DOWNLOAD_STATUS_ERROR, failReason);
                } else {
                    ...
                    if (downloadedSize == fileSize) {
                        ...
                        // error download
                        FileDownloadStatusFailReason failReason = new FileDownloadStatusFailReason(url, "the " +
                                "download " + "file size error !", FileDownloadStatusFailReason
    ...
    public void onDownloadConnected(ContentLengthInputStream inputStream, long startPosInTotal) {
        ...
        try {
            ...
            e.printStackTrace();
            // error download
            mFinishState = new FinishState(Status.DOWNLOAD_STATUS_ERROR, new FileDownloadStatusFailReason(getUrl(),
                    e));
        }
    }
    ...
    private boolean notifyStatusDownloading(int increaseSize) {
        try {
            ...
            if (downloadFileInfo == null) {
                ...
                increaseSize = 0;
                FileDownloadStatusFailReason failReason = new FileDownloadStatusFailReason(getUrl(), "the " +
            ...
            mFinishState = new FinishState(Status.DOWNLOAD_STATUS_ERROR, increaseSize, new
                    FileDownloadStatusFailReason(getUrl(), e));
            return false;
        }
    }
