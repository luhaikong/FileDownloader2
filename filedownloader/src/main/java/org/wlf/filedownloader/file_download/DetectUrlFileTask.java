...
class DetectUrlFileTask implements Runnable {
    ...
    public void run() {
         ...
         *
         * note: DetectBigUrlFileFailReason extends from DetectBigUrlFileFailReason, so they are compatible
         */
        ...
        DetectBigUrlFileFailReason failReason = null;
        ...
        try {
            // ------------start checking conditions------------
            {
                ...
                if (!UrlUtil.isUrl(mUrl)) {
                    // error, url illegal
                    failReason = new DetectBigUrlFileFailReason(mUrl, "url illegal !", DetectBigUrlFileFailReason
                            .TYPE_URL_ILLEGAL);
            ...
            if (redirectCount > MAX_REDIRECT_TIMES) {
                ...
                failReason = new DetectBigUrlFileFailReason(mUrl, "over max redirect:" + MAX_REDIRECT_TIMES + "!",
                        DetectBigUrlFileFailReason.TYPE_URL_OVER_REDIRECT_COUNT);
            ...
            switch (conn.getResponseCode()) {
                ...
                case HttpURLConnection.HTTP_NOT_FOUND:
                    ...
                    failReason = new DetectBigUrlFileFailReason(mUrl, "url file does not exist !",
                            DetectBigUrlFileFailReason.TYPE_HTTP_FILE_NOT_EXIST);
                    break;
                // other, ResponseCode error
                default:
                    // error ResponseCode error
                    failReason = new DetectBigUrlFileFailReason(mUrl, "ResponseCode:" + conn.getResponseCode() + " " +
                            "error, can not read data !", DetectBigUrlFileFailReason.TYPE_BAD_HTTP_RESPONSE_CODE);
                    break;
            }
        } catch (Exception e) {
            e.printStackTrace();
            // cast Exception to DetectBigUrlFileFailReason
            failReason = new DetectBigUrlFileFailReason(mUrl, e);
        } finally {
            ...
            // ------------start notifying caller------------
            {
                ...
                if (!isNotify) {
                    if (failReason == null) {
                        failReason = new DetectBigUrlFileFailReason(mUrl, "the file need to download may not access !",
                                DetectBigUrlFileFailReason.TYPE_UNKNOWN);
                    }
                    // error occur
