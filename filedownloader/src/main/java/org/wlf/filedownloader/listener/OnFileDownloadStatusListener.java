    ...
    public static class FileDownloadStatusFailReason extends HttpFailReason {
        ...
        protected void onInitTypeWithFailReason(FailReason failReason) {
            ...
            if (failReason instanceof HttpFailReason) {
                ...
                else if (failReason instanceof DetectBigUrlFileFailReason) {

                    DetectBigUrlFileFailReason detectUrlFileFailReason = (DetectBigUrlFileFailReason) failReason;
                    type = detectUrlFileFailReason.getType();

                    if (DetectBigUrlFileFailReason.TYPE_BAD_HTTP_RESPONSE_CODE.equals(type)) {
                        setType(TYPE_BAD_HTTP_RESPONSE_CODE);
                    } else if (DetectBigUrlFileFailReason.TYPE_HTTP_FILE_NOT_EXIST.equals(type)) {
                        setType(TYPE_FILE_NOT_DETECT);
                    } else if (DetectBigUrlFileFailReason.TYPE_URL_ILLEGAL.equals(type)) {
                        setType(TYPE_URL_ILLEGAL);
                    } else if (DetectBigUrlFileFailReason.TYPE_URL_OVER_REDIRECT_COUNT.equals(type)) {
                        setType(TYPE_URL_OVER_REDIRECT_COUNT);
                    }
                }
